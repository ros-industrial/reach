...
namespace moveit_reach_plugins
{
  ...
  namespace utils
  {
                            ...
                            std::vector<double> &input_subset)
    {
      ...
      {
        RCLCPP_ERROR(LOGGER, "Seed pose size was not at least as large as the number of joints in the planning group");
        return false;
      }
      ...
      for (const std::string &name : joint_names)
      {
        ...
        if (it == input.end())
        {
          RCLCPP_ERROR_STREAM(LOGGER, "Joint '" << name << "' in the planning group was not in the input map");
          return false;
        }
        else
        {
