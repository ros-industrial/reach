...
bool fromFile(const std::string &path, T &msg) {
  ...
  if (!ifs) {
    RCLCPP_INFO(rclcpp::get_logger("serialization_utils"),
                "Stream '%s' does not exist!", path.c_str());
    return false;
  }
  ...
  ser_msg->get_rcl_serialized_message().buffer_capacity =
      file_size * sizeof(uint8_t);
  auto serializer = rclcpp::Serialization<T>();
  serializer.deserialize_message(ser_msg, &msg);
  //      RCLCPP_INFO(rclcpp::get_logger("serialization_utils"), "Successfully
  //      serialized from file!");
  return true;
}
